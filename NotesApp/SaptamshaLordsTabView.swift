...
struct SaptamshaLordsTabView: View {
    ...
    var body: some View {
        NavigationView {
            ScrollView {
                let d7 = VargaCalculatorIOS.computeD7(planetPositions: planetPositions, ascendant: ascendant)
                VStack(spacing: 12) {
                    sectionHeader("D7 Ascendant: \(d7.ascSign)")
                    ForEach(d7.entries) { e in
    ...
    private func sectionHeader(_ text: String) -> some View {
        HStack { Text(text).font(.headline); Spacer() }
    }
